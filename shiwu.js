...
Page({
  ...
  getlosesList(){
    ...
    //   })
    // })
    wx.cloud.database().collection('school_loses').orderBy('time','desc').where({
  ...
  deleteAction(event){
    ...
    var that = this;
    wx.cloud.database().collection('school_loses').doc(event.currentTarget.dataset.id).remove({
      success(res){
        console.log(res)
        wx.showToast({
          title: '删除成功！',
  ...
  pirzeAction(event){
    if(app.globalData.userInfo == null){
      ...
      console.log(event.currentTarget.dataset.id)
      var that = this;
      wx.cloud.database().collection('school_loses').doc(event.currentTarget.dataset.id).get({
        success(res){
          ...
          if(tag){
            ...
            console.log(action)
            wx.cloud.database().collection('school_loses').doc(event.currentTarget.dataset.id).update({
              data: {
