...
Page({
  ...
  getlosesList(){
    ...
    //   })
    // })
    wx.cloud.database().collection('school_ershou').orderBy('time','desc').where({
  ...
  deleteAction(event){
    ...
    var that = this;
    wx.cloud.database().collection('school_ershou').doc(event.currentTarget.dataset.id).remove({
      success(res){
        console.log(res)
        wx.showToast({
          title: '删除成功！',
  ...
  pirzeAction(event){
    if(app.globalData.userInfo == null){
      ...
      console.log(event.currentTarget.dataset.id)
      var that = this;
      wx.cloud.database().collection('school_ershou').doc(event.currentTarget.dataset.id).get({
        success(res){
          ...
          if(tag){
            ...
            console.log(action)
            wx.cloud.database().collection('school_ershou').doc(event.currentTarget.dataset.id).update({
              data: {
  ...
  onShareAppMessage(event){
    ...
    if(event.from == 'button'){
      ...
      return {
        title: this.data.losesList[index].text,
        imageUrl: this.data.losesList[index].images[0],
        path:'pages/detail/detail?id=' + this.data.losesList[index]._id
      }
    }
    if(event.from == 'menu'){
      return {
